...
with DAG(
    dag_id       = "datalake-tds-batch-pipeline-enty-batch",
    start_date   = datetime(2023, 3, 25),
    schedule = Variable.get("EMR_BATCH_SCHEDULE_ENTY"),
    ...
    sensor_tds = ExternalTaskSensor(
        task_id='external_sensor_task_tds_enty',
        external_dag_id='datalake-batch-pipeline-enty',  # Specify the DAG  ID
    ...
    for t_list in table_list:
        ...
        with TaskGroup(group_id=task_group) as task_group:
            ...
            tds_transform=EmrContainerOperator(
                ...
                "entryPointArguments": ["--config",f"{get_table_config(src_schema, src_table, tgt_schema, tgt_table)}","--jsonfile",f"s3://{TDS_JOB_FILE_BUCKET_NAME}/tds/config/enty/{tgt_table}_config.json","--spark_conf_options", str(TDS_SPARK_CONF_OPTIONS)],
